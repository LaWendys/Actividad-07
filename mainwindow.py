...
class MainWindow(QMainWindow):
    ...
    def action_abrir_archivo(self):
        ...
        if self.administrador.abrir(ubicacion):
            QMessageBox.information(self,"Exito", "Se abrio el Archivo de" + ubicacion)
        else:
            QMessageBox.information(self,"Error","Error al Abrir el Archivo" + ubicacion) 
        
    @Slot()
    def action_guardar_archivo(self):
        ubicacion = QFileDialog.getSaveFileName(self, 'Guardar Archivo','.','JSON (*.json)')[0]
        if self.administrador.guardar(ubicacion):
            QMessageBox.information(self, "Exito", "Se pudo Crear el archivo en" + ubicacion)
        else: 
            QMessageBox.information(self, "Error", "No se pudo crear el Archivo D:" + ubicacion)
